...
- **streaming output** - концепция, при которой воркер дернет ручку и будет долго отправлять
  данные в очередь, пока они там будут, в качестве примера тут сделаем джобу, отправляющую
  число каждую секунду, начиная с 1, а пайплайн будет считать 3x+2.